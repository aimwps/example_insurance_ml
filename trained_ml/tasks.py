...
def train_model(data_df, preprocessing, training_settings, ml_model):
    #combine the model with preproccessing into a pipeline
    training_pipeline = pipeline.make_pipeline(preprocesing, ml_model)
...
def train_model_from_db(training_settings):
    ...
    if training_settings['ml_model'] in model_dict['tree'].keys():
        ml_model = model_dict['tree'][training_settings['ml_model']]
        preprocessing = build_tree_training(training_settings)
    elif training_settings['ml_model'] in model_dict['tree'].keys():
        ml_model = model_dict['mult'][training_settings['ml_model']]
        preprocessing = build_mult_training(training_settings)
    else:
        print("We didn't recognise the model type")

    data = get_data_from_api(training_settings)

    results = train_model(data, preprocessing, training_settings, ml_model)
